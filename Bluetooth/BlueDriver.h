...
private:
    ...
    class Channel : public MessageExchangeType<Core::SerialPort, Exchange::Bluetooth> {
    private:
        Channel() = delete;
    ...
    public:
        Channel(Driver& parent, const string& name, const Core::SerialPort::BaudRate baudrate, const Core::SerialPort::flowControl flowControl)
            : BaseClass ()
            , _parent(parent) {
            BaseClass::Configure(name, baudrate, flowControl, 64, 64);
        }
        virtual ~Handler() {
        }
...
public:
    using namespace Exchange::Bluetooth = Message;

public:
    Driver(const string& port, const BaudRate baudRate, const Core::SerialPort::flowControl flowControl, const bool sendBreak) :
    ...
    virtual ~Driver() {
        port.Close(Core::infinite);
    }

public:
    uint32_t Initialize(const BaudRate baudRate, const unsigned long flags, const iint protocol) {
        ...
        if (_port.IsOpen() == true) {
	        ...
	        _port.Flush();
                result = _port.SetBaudrate(baudRate);
...
private:
    Core::SerialPort _port;
};

class EXTERNAL Broadcom43XX : public Driver {
private:
    Broadcom43XX() = delete;
...
private:
    string FindFirmware(const string& directory)
    {
        Core::Dire
        DIR *dir;
        int ret = -1;
    ...
    uint32_t Reset() {
        Message::Response response (HCI_COMMAND_PKT, 0x030C);
        uint32_t result = Exchange (Message::Request(HCI_COMMAND_PKT, 0x030C, 0, nullptr), response, 500);
    ...
    uint32_t LoadName() {
        Message::Response response (HCI_COMMAND_PKT, 0x140C);
        uint32_t result = Exchange (Message::Request(HCI_COMMAND_PKT, 0x140C, 0, nullptr), response, 500);
	...
	if ((result == Core::ERROR_NONE) && (response[0] != CMD_SUCCESS)) {
        ...
        if (result == Core::ERROR_NONE) {
            _name = string(&(response[1]), response.DataLength()-1); 
        }
        return (result);
    }
    uint32_t SetClock(const uint8_t clock)
    {
        Message::Response response (HCI_COMMAND_PKT, 0x45fc);
        uint32_t result = Exchange (Message::Request(HCI_COMMAND_PKT, 0x45fc, 1, &clock), response, 500);
    ...
    uint32_t SetSpeed(const Core::SerialPort::Baudrate baudrate)
    {
        ...
        if (result == Core::EROR_NONE) {
           ...
           Message::Response response (HCI_COMMAND_PKT, 0x18fc);
           uint32_t result = Exchange (Message::Request(HCI_COMMAND_PKT, 0x18fc, sizeof(data), data), response, 500);
    ...
    static int MACAddress (const uint8_t length, const uint8_t address[])
    {
        uint8_t data[6]; 
        ::memcpy (data, address, min(length, sizeof(data)));
        ::memset (&(data[length]), 0, sizeof(data) - std::min(length, sizeof(data)));

        Message::Response response (HCI_COMMAND_PKT, 0x01fc);
        uint32_t result = Exchange (Message::Request(HCI_COMMAND_PKT, 0x01fc, sizeof(data), address), response, 500);
    ...
    uint32_t Firmware (const TCHAR filename[]) {
        ...
        if (fd >= 0) {
            Message::Response response (HCI_COMMAND_PKT, 0x2efc);
            uint32_t result = Exchange (Message::Request(HCI_COMMAND_PKT, 0x2efc, 0, nullptr), response, 500);
            ...
            if (result == Core::ERROR_NONE) {
                ...
                while ((result == Core::ERROR_NONE) && ((loaded = read(fd, tx_buf, 3)) > 0)) {
                    ...
                    if (read(fd, tx_buf, len) < 0) {
                        ...
                        Message::Response response (HCI_COMMAND_PKT, code);
                        result = Exchange (Message::Request(HCI_COMMAND_PKT, code, len, tx_buf), response, 500);
                        Flush();
                    }
                }

                if ((loaded != 0) && (result == Core::ERROR_NONE)) {
                    result = Core::ERROR_ACKNOWLEDGE;
                }
    ...
    virtual uint32_t Prepare() override {
        ...
        if (result == Core::ERROR_NONE) {
            ...
            if (result == Core::ERROR_NONE) {
                result = LocatePatch();
                if (result == Core::ERROR_NONE) {
                    result = LoadFirmware(firmwareFile.c_str());
                    if (result == Core::ERROR_NONE) {
                        result = Reset();
                    }
                }
